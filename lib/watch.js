...
function composeMessage(names) {
  return makeArray(names).map(function(n) {
    if (!is.exists(n)) return ['remove', n];
...
Watcher.prototype.add = function(watcher, info) {
  ...
  var callback = function(evt, name) {
    if (info.options.recursive) {
      var has = ['darwin', 'win32'].includes(process.platform);
      if (!has) {
        ...
        // watch new created directory
          ...
          self.watchItem(name, info.options);
        }
      }
    }

    // process guard
    guard(info.options.filter)(name, function () {
  ...
  watcher.on('error', function(err) {
    if (os.platform() == 'win32' && err.code == 'EPERM') {
      watcher.emit('change', 'remove', info.fpath && '');
...
Watcher.prototype.watchItem = function(item, options, fn) {
  var self = this;
  var dir;
  var watcher;
  var opts;
  ...
  if (is.file(item)) {
    ...
    opts = Object.assign({}, options, {
      recursive: false,
      filter: null, // yes, need to set to item's name!!!
      encoding: 'utf8'
    });

    watcher = fs.watch(dir, opts);
    self.add(watcher, {
      type: 'file',
      fpath: dir,
      ...
      // no COMPARENAME -- put this inside filter!!!
    });
    if (is.func(fn)) {
      self.on('change', fn);
    }
    ...
    while (!is.directory(dir)) dir = globParent(dir);
    options.filter = /^app(?:\/|$)/; /* make sure we match the newly materialized fragment */
...
function watch(fpath, options, fn) {
  ...
  watcher.watchItem(fpath, options, fn);
